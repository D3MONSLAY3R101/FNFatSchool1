...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'just-a-burning-memory-8bit':
				dialogue = CoolUtil.coolTextFile(Paths.txt('just-a-burning-memory-8bit/Dialogue'));
			case 'just-a-burning-memory':
				dialogue = CoolUtil.coolTextFile(Paths.txt('just-a-burning-memory/Dialogue'));
			case 'we-dont-have-many-days':
				dialogue = CoolUtil.coolTextFile(Paths.txt('we-dont-have-many-days/Dialogue'));
			case 'childishly-fresh-eyes':
				dialogue = CoolUtil.coolTextFile(Paths.txt('childishly-fresh-eyes/Dialogue'));
			case 'all-that-follows-is-true':
				dialogue = CoolUtil.coolTextFile(Paths.txt('all-that-follows-is-true/Dialogue'));
			case 'misplaced-in-time-8bit':
				dialogue = CoolUtil.coolTextFile(Paths.txt('misplaced-in-time-8bit/Dialogue'));
			case 'libets-delay-8bit':
				dialogue = CoolUtil.coolTextFile(Paths.txt('libets-delay-8bit/Dialogue'));
		}
		...
		else if (SONG.song.toLowerCase() == 'just-a-burning-memory-8bit' || SONG.song.toLowerCase() == 'misplaced-in-time-8bit')
		{
			curStage = 'school';
			...
			if (SONG.song.toLowerCase() == 'misplaced-in-time-8bit')
			{
				bgGirls.getScared();
			}
		...
		else if (SONG.song.toLowerCase() == 'libets-delay-8bit')
		{
			curStage = 'schoolEvil';
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		if (SONG.song.toLowerCase() == 'misplaced-in-time-8bit' || SONG.song.toLowerCase() == 'libets-delay-8bit')
		{
			remove(black);

			if (SONG.song.toLowerCase() == 'libets-delay-8bit')
			{
				add(red);
			}
		}
